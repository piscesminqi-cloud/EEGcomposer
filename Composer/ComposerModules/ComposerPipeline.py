    ...
    def __call__(
        ...
        for t in self.scheduler.timesteps:
            ...
            cond_pixel_values = torch.cat(
                [clip_image_embeds] * 2) if clip_image_embeds is not None else None
            cond_prompt = torch.cat(
                [text_embeddings] * 2) if text_embeddings is not None else None
            cond_color = torch.cat([color] * 2) if color is not None else None

            # 准备视觉条件（应用dropout）
            cond_local_conditions = self.local_condition_proj(
                sketch=torch.cat([sketch] * 2) if sketch is not None else None,
                instance=torch.cat([instance] * 2) if instance is not None else None,
                depth=torch.cat([depth] * 2) if depth is not None else None,
                intensity=torch.cat([intensity] * 2) if intensity is not None else None
            )
            ...
            # 将视觉条件与latents合并
            cond_color = cond_color / torch.norm(cond_color, p=2, dim=-1, keepdim=True)
