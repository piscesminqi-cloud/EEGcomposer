...
class EEG2FeatureModel(nn.Module):
    def __init__(self, input_dim, output_dim, feature_shape=None):
        ...
        self.input_proj = nn.Linear(input_dim, 128)  # 增加隐藏维度
        
        # Transformer编码器 - 增加层数和注意力头
        encoder_layers = TransformerEncoderLayer(
            d_model=128,          # 增加模型容量
            nhead=8,
            dim_feedforward=2048, # 增加前馈网络维度
            dropout=0.2,          # 增加dropout防止过拟合
            activation="gelu",
            batch_first=True      # 使用batch_first提高效率
        )
        self.transformer = TransformerEncoder(encoder_layers, num_layers=6)  # 增加层数
    ...
    def forward(self, x):
        ...
        # 投影到更高维度
        x = self.input_proj(x)  # (batch, timesteps, features)
        
        # Transformer处理
        x = self.transformer(x)  # (batch, timesteps, features)
        ...
        # 加权求和
        x = torch.bmm(attn_weights, x).squeeze(1)  # (batch, features)
...
def train_all_subjects_models():
    ...
    for feature_type in config.feature_dims:
        ...
        optimizer = optim.Adam(model.parameters(), lr=config.lr, weight_decay=1e-5)  # 增加权重衰减
