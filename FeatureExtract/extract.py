""" 将 EEG 信号通过线性回归（Ridge Regression）映射到图像的 latent 表征（npy 向量），并保存预测结果。"""
import numpy as np
import os
import pickle
...
def load_latent_data(latent_type, is_train=True):
    ...
    if latent_type == 'clip_image':
        if is_train:
            ...
            path = f'features_test/all_image_test.npy'
            data = np.load(path)  # 形状(200, 768)
        return data
    
    elif latent_type == 'clip_text':
        if is_train:
            path = f'features_train/all_text_train.npy'
            data = np.load(path)  # 形状(16540, 77, 768)
        else:
            path = f'features_test/all_text_test.npy'
